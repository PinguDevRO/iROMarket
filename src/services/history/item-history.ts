...
const GetItemHistory = async (server: string, kind: string, page: number, page_size: number, itemName?: string, itemId?: number): Promise<ListingResponse> => {
    ...
    if(itemName !== undefined && itemName.length > 0){
        const response: AxiosResponse<ListingResponse> = await AxiosGet(`${url}/archive/listing?server=${server}&q=${itemName}&t=${kind}&page=${page + 1}&page_size=${page_size}`);
        return response.data;
    }
    if(itemId !== undefined){
        const response: AxiosResponse<ListingResponse> = await AxiosGet(`${url}/archive/listing?server=${server}&id=${itemId}&t=${kind}&page=${page + 1}&page_size=${page_size}`);
        return response.data;
    }
    const response: AxiosResponse<ListingResponse> = await AxiosGet(`${url}/archive/listing?server=${server}&t=${kind}&page=${page + 1}&page_size=${page_size}`);
    return response.data;
};
