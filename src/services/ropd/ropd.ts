...
const GetRoPD = async (server: string, page: number, page_size: number, playerName?: string, accountId?: number): Promise<ROPDResponse> => {
    ...
    if(playerName !== undefined && playerName.length > 0){
        const response: AxiosResponse<ROPDResponse> = await AxiosGet(`${url}/ropd?server=${server}&q=${playerName}&page=${page + 1}&page_size=${page_size}`);
        return response.data;
    }
    if(accountId !== undefined){
        const response: AxiosResponse<ROPDResponse> = await AxiosGet(`${url}/ropd?server=${server}&id=${accountId}&page=${page + 1}&page_size=${page_size}`);
        return response.data;
    }
    const response: AxiosResponse<ROPDResponse> = await AxiosGet(`${url}/ropd?server=${server}&page=${page + 1}&page_size=${page_size}`);
    return response.data;
};
